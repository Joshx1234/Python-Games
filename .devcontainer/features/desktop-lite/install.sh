...
# Remove stale Yarn apt source that ships in the base image — its GPG key
# (NO_PUBKEY 62D54FD4003F6525) is no longer valid and causes apt-get update
# to fail with an unsigned-repository error, crashing the feature install.
